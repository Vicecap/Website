...
async function loadMatchData(updateOnly = false) {
  const matchSlug = getParam("matchSlug");
const eventId = getParam("eventId");
const country = getParam("country", "europe");
const league = getParam("league", "champions-league");
...
// Update only scores/status every 5 seconds
setInterval(() => {
  loadMatchData(true);
}, 5000);
